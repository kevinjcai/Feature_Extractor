...
def vid_ex(logger, data_list, output, gpu):
    opt = parse_args()
    logger.info(opt)
    ...
    for vline in tqdm(data_list):
        ...
        np.save(os.path.join(output, feat_file), video_feat.asnumpy())
...
def video_feat(txt_name, feature_path, gpu = -1):
    ...
    extracter_path = "vid_feat.py"
    
    # logging.basicConfig()
    logger = logging.getLogger('logger')
    # logger.setLevel(logging.INFO)
    vid_ex(logger, txt_name, vid_feat_path, gpu)
